...
class ChooseK(Propagator):
    ...
    def on_assignment(self, lit, fixed=False):
        ...
        if CHECK_AT_MOST_CONDITION and len(self.assigned_positive) == self.k:
            must_be_false = set(vars_ids)-self.assigned_positive-self.assigned_negative
            if must_be_false:
                clause_prefix = [-t for t in sorted(self.assigned_positive)]  # ¬t1 ∨ ... ∨ ¬tk ∨ ¬v
                for v in must_be_false:
                    impl = -v
                    if impl not in self.reason:
                        self.queue.append(impl)
                        self.reason[impl] = clause_prefix + [-v]
                        self.reason_at_level[self.level].add(impl)

        if CHECK_AT_LEAST_CONDITION and len(self.assigned_negative) == self.n-self.k:
            must_be_true = set(vars_ids)-self.assigned_positive-self.assigned_negative
            if must_be_true:
                clause_prefix = [t for t in sorted(self.assigned_negative)]  # t1 ∨ ... ∨ tk ∨ v
